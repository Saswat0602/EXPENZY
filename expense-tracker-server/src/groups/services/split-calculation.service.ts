...
export interface SplitValidationResult {
    isValid: boolean;
    status: 'valid' | 'sum_mismatch' | 'percentage_mismatch' | 'invalid_participants' | 'overpayment' | 'negative_amount';
    message?: string;
    difference?: number;
}
...
export class SplitCalculationService {
    ...
    calculateEqualSplit(
        ...
        if (!remainderAssigned && remainder > 0) {
            ...
            splits[0].percentage = Math.round((splits[0].amountOwed / totalAmount) * 10000) / 100;
        }
    ...
    calculateExactSplit(
        ...
        if (participants.some(p => p.amount === undefined || p.amount === null)) {
            throw new BadRequestException('All participants must have a valid amount for exact split');
        }
        ...
        if (zeroAmounts.length === participants.length) {
            throw new BadRequestException('At least one participant must have an amount greater than ₹0');
        }
        ...
        return participants.map(participant => ({
            ...
            percentage: totalAmount > 0 ? Math.round((participant.amount! / totalAmount) * 10000) / 100 : 0,
    ...
    calculatePercentageSplit(
        ...
        if (participants.some(p => p.percentage === undefined || p.percentage === null)) {
            throw new BadRequestException('All participants must have a valid percentage');
        }
        ...
        if (validParticipants.length === 0) {
            throw new BadRequestException('At least one participant must have a percentage greater than 0%');
        }

        const totalPercentage = validParticipants.reduce((acc, p) => acc + (p.percentage || 0), 0);
    ...
    calculateSharesSplit(
        ...
        if (validParticipants.length === 0) {
            throw new BadRequestException('At least one participant must have shares greater than 0');
        }

        const totalShares = validParticipants.reduce((acc, p) => acc + (p.shares || 0), 0);
        const amountPerShare = totalAmount / totalShares;

        // Calculate amounts from shares
        const splits: CalculatedSplit[] = validParticipants.map(participant => {
            const calculatedAmount = Math.round(amountPerShare * participant.shares! * 100) / 100;
            const percentage = Math.round((participant.shares! / totalShares) * 10000) / 100;
